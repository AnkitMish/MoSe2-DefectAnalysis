...
int write_features(a_coodrinates *input_atoms, char *filename, char *filename1, int Natoms_tot, float *box,int Natoms)
{
    FILE *fp, *fp1;
    int count=0;
    ...
    for (int i = 0; i < Natoms_tot; i++)
    {
        ...
        {
            fprintf(fp,"%d \t %12.6f \t %12.6f \t %12.6f \t %12.6f \t %12.6f \t",input_atoms[i].atype,input_atoms[i].loc[0], input_atoms[i].loc[1],input_atoms[i].loc[2],input_atoms[i].property,input_atoms[i].property2);
            fprintf(fp,"\n");

            fprintf(fp1, "%d \t %12.6f \t %12.6f \t %12.6f \t %12.6f\n", input_atoms[i].atype, input_atoms[i].loc[0], input_atoms[i].loc[1],input_atoms[i].loc[2],input_atoms[i].property2);
...
int viz_tensor(float **tensor_mo,float **tensor_set,float **tensor_seb)
{
    ...
    for (int ii=0;ii<ngrids;ii++)
    {
        for (int jj=0;jj<ngrids;jj++) fprintf(fp1,"%12.6f ",tensor_mo[ii][jj]);
    ...
    for (int ii=0;ii<ngrids;ii++)
    {
        for (int jj=0;jj<ngrids;jj++) fprintf(fp2,"%12.6f ",tensor_set[ii][jj]);
    ...
    for (int ii=0;ii<ngrids;ii++)
    {
        for (int jj=0;jj<ngrids;jj++) fprintf(fp3,"%12.6f ",tensor_seb[ii][jj]);
...
int main(int argc, char* argv[])
{
    ...
    int H_frac=0, T_frac=0, maxlimit=50000, itag;
    float prob_val, prob=1.0;
    int cur_element=3;
    ...
    fprintf(stdout,"Input parameters: %s %d\n", filename, maxlimit/*, cur_element*/);
    ...
    for(int i=0;i<mdatom_info.Natoms;i++)
    {
	   ...
	   itag = 0;
       	if(input_atoms[i].atype == 1)
        {
                ...
                else if (input_atoms[i].property2 == 2)
                {
                    prob_val =((double)rand())/((double)RAND_MAX);
                    if (prob_val <= prob && N_moatoms <= maxlimit) {itag = 1;}
                }
	   }

	   if (itag == 1 /*&& input_atoms[i].property2 <= cur_element*/) 
       {
	       makeneighboutlist_peratom(input_atoms,i, mdatom_info.Natoms, mdatom_info.boxmd, mdatom_info.halfboxmd, mdatom_info.cellsize, mdatom_info.ng, mdatom_info.rcutoffsq, mdatom_info.llst,mdatom_info.lshd, &nlist);
    ...
    write_features(input_atoms,"dcnn_feature.txt","test.xyz",mdatom_info.Natoms,mdatom_info.boxmd,N_moatoms);
